...
- Flask art synchronization now includes PoE life, mana, hybrid, and utility flask classes. The five flask cells are centered 1-by-2 portrait rectangles, and the amulet/right-ring cells span the previously unused column beside the offhand.
...
- Before calculating, the worker explicitly enables the socket group referenced by `<Build mainSocketGroup>`. PoB exports can retain cached headline DPS after that selected group is unchecked; without this normalization a clean headless calculation silently falls back to another enabled group (the reproduced Kinetic Blast build fell back to Frostblink, changing 5.98M to about 58K).
