...
The root URL and `GET /health` are public status endpoints. `POST /evaluate` requires the `ENGINE_TOKEN` bearer token. The service refuses evaluation requests if that secret is missing.
